...
import { ConfirmActionDialog } from "@/features/kuan-plan/PlanDialogs";
...
function GuardioesPage() {
  ...
  const [pendingStatus, setPendingStatus] = useState<{ id: string; status: GuardianStatus } | null>(
    null,
  );
  ...
  async function persistStatus(id: string, status: GuardianStatus) {
    try {
  ...
  return (
    <>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-[color:var(--ivory-dim)]">
                      Este link é para clientes. Não exige login. Cliente não recebe convite de
                      conta; cliente recebe link público de atendimento.
                    </p>
                    <p className="mt-1 text-xs text-[color:var(--ivory-dim)]">
                    ...
                    {guardian.metadata?.public_page_blueprint && (
                      <p className="mt-1 text-xs text-[color:var(--ivory-dim)]">
                        Proposta pública:{" "}
                        {guardian.metadata.public_page_blueprint.status ?? "draft"}. Nada é
                        publicado automaticamente sem revisão.
                      </p>
                    )}
                  </div>
      ...
      <ConfirmActionDialog
        ...
        title="Confirmar mudança pública"
        description={
