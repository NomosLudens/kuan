...
AS $$
  SELECT EXISTS (
    ...
    JOIN public.business_contexts bc
      ON bc.id = p_business_context_id
...
DROP POLICY IF EXISTS kuanyin_business_plans_owner_all ON public.kuanyin_business_plans;
...
AS $$
DECLARE
  v_old public.kuanyin_plan_decisions;
  v_new public.kuanyin_plan_decisions;
BEGIN
  SELECT *
  INTO v_old
